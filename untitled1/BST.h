...
class BST {
    ...
    friend class BSTItrLevel<Comparable>;
    friend class iteratorBST<Comparable>;
    int contNodes(BinaryNode<Comparable> *node) const;
public:
...
//TODO
template <class Comparable>
int BST<Comparable>::contNodes(BinaryNode<Comparable>*nod) const{
    if(nod==NULL)
        return 0;
    return 1+ contNodes(nod->right)+contNodes(nod->left);
}


template <class Comparable>
int BST<Comparable>::size(const Comparable& el) const {
    auto place=find(el, root);
    if(place==NULL){
        return -1;
    }
    return contNodes(place)-1;
}
